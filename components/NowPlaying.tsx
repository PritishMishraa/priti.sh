...
export default function NowPlaying() {
  ...
  return (
    <div className="flex flex-row-reverse items-center w-full mb-8 space-x-0 sm:flex-row sm:space-x-2">
      {data?.songUrl ? (
        <Image className='mr-1 rounded-full animate-spin-slow' height={40} width={40} unoptimized src={data.albumImageUrl} alt={data.title} />
      ) : (
        <svg className="h-6 w-6 ml-auto mt-[-2px]" viewBox="0 0 168 168">
          <path
            fill="#1ED760"
