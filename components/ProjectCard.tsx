...
export default function ProjectCard({ title, description, gradient, tech, githubLink, hostedLink, isMigrating }) {
      ...
      <div className="flex flex-col justify-between p-4 bg-white rounded-lg dark:bg-gray-900">
        <div className="flex flex-col justify-between">
          <div className="flex justify-between mb-2 sm:mb-3">
            ...
            {isMigrating && <span className="inline-flex items-center bg-red-100 text-red-800 text-xs font-medium px-2.5 py-0.5 rounded-lg dark:bg-red-900 dark:text-red-300 mr-2">
        ...
        <div className="flex items-center justify-between text-xs text-gray-800 dark:text-gray-200">
          <div className='flex md:hidden'>
            {slicedTechArray.map((title: string) => {
              return (
                <span key={title} className="capsize align-baseline rounded-lg bg-gray-200 dark:bg-gray-800 p-1.5 m-0.5">
                  {title}
                </span>
              )
            })}
            <div className="capsize align-baseline rounded-lg bg-gray-200 dark:bg-gray-800 p-1.5 m-0.5">...</div>
          </div>
